import { Flag, HelpCircle, Sparkles } from "lucide-react";
...
export function QuizCard({ pergunta, mostrarResposta }: Props) {
  if (!pergunta) {
    return (
      <div className="glass-panel p-8 md:p-12 rounded-3xl text-center border border-purple-500/20 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center space-y-4 relative z-10">
          <div className="p-4 bg-purple-500/10 text-purple-400 rounded-full border border-purple-500/20 shadow-[0_0_15px_rgba(168,85,247,0.155)]">
            <Flag size={32} className="animate-pulse" />
          </div>
          <h2 className="text-2xl md:text-3xl font-bold text-slate-100 font-sans">Fim das perguntas!</h2>
          <p className="text-slate-400 text-sm">Todas as perguntas da rodada foram concluídas.</p>
        </div>
      </div>
    );
  }
